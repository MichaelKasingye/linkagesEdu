...
setInfo(snapshot.docs.map(doc => ({id: doc.id,data:doc.data()})).filter(filterData => filterData.id === id));
    ...
    return (
        ...
        <div className="">
            <h2>Profile</h2>
         {/* <Image  src={info[0].data.imageUrl}   loader={({ src, width, quality }) => {
    return `${info[0].data.imageUrl}`
  }} alt="Picture of the author" width={400}height={400} /> */}
  <img src={info[0].data.imageUrl} alt="img" width="300" height="300"/>
         ...
         {/*{info[0].data.projectLink} */}
          <p><strong> <a  href={info[0].data.projectLink} target="_blank" rel="noopener noreferrer">jumia clone</a> </strong></p>
...
<img src={info[0].data.projectImgLink} alt="img" width="300" height="300"/>
    ...
    <h2>Certificates</h2>
         ...
         {/* <p>{info[0].data.certificateName}</p> */}
         {/* <Link href={info[0].data.certificateImage}>Certificate link</Link> */}
        </div>
      ) : (
